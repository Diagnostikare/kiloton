...
export default function Chart({ height, weight }) {
  ...
  useEffect(() => {
    ...
    if (isMounted) {
      ...
      const calcImc = (weight / (heightInMeters * heightInMeters)).toFixed(2);
      if (!isNaN(calcImc)) setImc(calcImc);
    }
  }, [height, weight]);
      ...
      <div className={styles.graph}>
        <svg width="106" height="54" fill="none" viewBox="0 0 106 54">
          <g>
            ...
            />
            <path
              className={`${styles.path} ${imc >= 30 && styles.active}`}
        ...
        <span className={styles.state}>{getState(imc)}</span>
      </div>
      <div className={styles.data}>
        <span className={styles.value}>{imc}</span>
